...
function install(){
  ...
  function openSettings(){
    ...
    panel.innerHTML=`
      ...
      <section class="pp-settings-dialog" role="dialog" aria-modal="true" aria-labelledby="ppSettingsTitle">
        <div class="pp-settings-head">
          <div><div class="pp-settings-kicker">PRINTPROFIT</div><h2 id="ppSettingsTitle">Settings</h2><p>Manage the calculator display and quick actions.</p></div>
        ...
        <div class="pp-settings-body">
          <div class="pp-settings-card">
            ...
            <label class="pp-settings-switch"><input id="ppSettingsDark" type="checkbox" role="switch"><span></span></label>
          </div>
          <div class="pp-settings-card">
          ...
          <div class="pp-settings-note">Your calculator data is not changed just by opening Settings.</div>
        </div>
      </section>`;
    document.body.appendChild(panel);
    const dark=document.getElementById('dark');
    const settingsDark=document.getElementById('ppSettingsDark');
    if(settingsDark && dark){
      settingsDark.checked=dark.checked;
      settingsDark.addEventListener('change',()=>{
        dark.checked=settingsDark.checked;
    ...
    style.textContent=`
      ...
      .pp-settings-dialog{position:absolute;right:28px;top:78px;width:min(440px,calc(100vw - 32px));border:1px solid #315261;border-radius:16px;background:linear-gradient(180deg,#0c202b,#07131b);box-shadow:0 28px 80px #000b,0 0 34px #ff780014;color:#f5f8fb;overflow:hidden}
      .pp-settings-head{display:flex;align-items:flex-start;justify-content:space-between;gap:16px;padding:20px 20px 15px;border-bottom:1px solid #284553}
      .pp-settings-kicker{color:#ff7800;font-size:8px;font-weight:900;letter-spacing:.2em;margin-bottom:5px}
      .pp-settings-head h2{margin:0;font-size:22px}
      .pp-settings-head p{margin:5px 0 0;color:#8fa6b2;font-size:11px}
      .pp-settings-close{width:34px;height:34px;border:1px solid #355464;border-radius:9px;background:#091821;color:#dce7ec;font-size:22px;cursor:pointer}
      .pp-settings-close:hover{border-color:#ff7800;color:#fff}
      .pp-settings-body{padding:15px}
      .pp-settings-card{display:flex;align-items:center;justify-content:space-between;gap:14px;padding:14px;border:1px solid #284654;border-radius:12px;background:#081720;margin-bottom:10px}
      ...
      .pp-settings-note{padding:8px 4px 0;color:#718a98;font-size:9px;line-height:1.4}
      @media(max-width:650px){.pp-settings-dialog{left:12px;right:12px;top:74px;width:auto}.pp-settings-head{padding:16px}.pp-settings-body{padding:12px}.pp-settings-card{align-items:flex-start;flex-direction:column}.pp-settings-action{width:100%}}
    `;
    document.head.appendChild(style);
