...
function addQuickCards(main){
 ...
 wrap.querySelectorAll('[data-master-open]').forEach(btn=>btn.addEventListener('click',()=>{
  ...
  const tab=document.querySelector('.pp-step[data-tab="'+target+'"]')||document.querySelector('.pp-tab[data-tab="'+target+'"]');
  if(tab)tab.click();
...
.pp-master-quickcards{display:grid!important;grid-template-columns:repeat(4,minmax(0,1fr))!important;gap:16px!important;margin:20px auto 14px!important;max-width:1480px!important;padding:0 clamp(10px,2vw,28px)!important}
