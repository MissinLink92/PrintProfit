...
function readTime(text){
  const patterns=[
    ...
    /(?:total\s+)?estimated\s+(?:printing\s+)?time(?:\s*\([^)]*\))?\s*[:=]\s*([^;\r\n]+)/i,
...
function readPhysical(text){
  ...
  const diameterField=fieldBySemantic(['filament diameter','diameter']);
  const existingDiameter=num(diameterField?.value)||null;
...
function importGcode(file){
  ...
  file.text().then(text=>{
    ...
    if(status){
      ...
      if(found.length)status.textContent='✓ '+file.name+' — auto-filled '+found.join(' • ')+(source!=='G-code metadata'?' • '+source:'');
      else{const missing=[];if(!gotGrams)missing.push('filament usage');if(!gotTime)missing.push('print time');status.textContent='✓ '+file.name+' loaded — no readable '+missing.join(' or ')+' metadata found.';}
    }
    const calc=$('calc');if(calc)calc.click();
  }).catch(err=>{console.error('PrintProfit G-code reader error:',err);if(status)status.textContent='Could not read '+file.name+'. Please use a standard sliced G-code file.';});
}
function formatDuration(sec){sec=Math.max(0,Math.round(Number(sec)||0));const h=Math.floor(sec/3600),m=Math.floor((sec%3600)/60),s=sec%60;return h?`${h}h ${m}m`:m?`${m}m ${s}s`:`${s}s`;}
function preventLegacy(event,file){if(!file||!GCODE_EXT.has(ext(file.name)))return false;event.preventDefault();event.stopImmediatePropagation();const input=$('file');try{const dt=new DataTransfer();dt.items.add(file);input.files=dt.files;}catch(_){}importGcode(file);return true;}
function install(){
  const input=$('file');
