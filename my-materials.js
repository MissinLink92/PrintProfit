...
function openForm(existing){
 ...
 document.getElementById('ppMatBrand').value=existing?.brand||'';
...
function saveForm(){
 ...
 const brand=document.getElementById('ppMatBrand').value.trim(),colour=document.getElementById('ppMatColour').value.trim();
...
function open(){
 ...
 if(!panel){
  ...
  <div class="pp-material-form-grid"><label>Material name<input id="ppMatName" placeholder="e.g. eSUN PLA+ Black"></label><label>Type<select id="ppMatType"><option>Filament</option><option>Resin</option></select></label><label>Brand<input id="ppMatBrand" placeholder="Optional"></label><label>Colour<input id="ppMatColour" placeholder="Optional"></label><label><span id="ppMatAmountLabel">Spool weight (g)</span><input id="ppMatAmount" type="number" min="0" step=".01" value="1000"></label><label>Package cost (£)<input id="ppMatCost" type="number" min="0" step=".01" value="20"></label></div>
...
document.addEventListener('change',e=>{if(e.target?.id==='ppMatType'){const l=document.getElementById('ppMatAmountLabel');if(l)l.textContent=e.target.value==='Resin'?'Bottle amount (ml)':'Spool weight (g)'}});
