(()=>{
  ...
  function install(){
    ...
    style.textContent=`
/* PrintProfit progress workspace V5 */
...
/* The lower navigation is intentionally a progress indicator, not another card row. */
.pp-progress{display:grid;grid-template-columns:1fr 1fr 1fr;align-items:center;width:100%;margin:0 0 12px;padding:0;background:rgba(9,24,33,.86);border:1px solid var(--line);border-radius:12px;overflow:hidden;box-shadow:0 8px 24px #0004}
.pp-step{position:relative;min-height:58px;border:0;background:transparent;color:var(--muted);padding:8px 14px;display:flex;align-items:center;justify-content:center;gap:10px;font:700 13px Inter,Segoe UI,system-ui,sans-serif;cursor:pointer;transition:background .18s ease,color .18s ease}
.pp-step:not(:last-child)::before{content:"";position:absolute;right:0;top:50%;width:1px;height:30px;transform:translateY(-50%);background:var(--line)}
...
.pp-step.active::after{content:"";position:absolute;left:12px;right:12px;bottom:0;height:3px;background:var(--accent);border-radius:3px 3px 0 0}
.pp-step.complete .pp-step-number{background:#ff780019;border-color:#ff780055;color:var(--accent)}
.pp-step-number{width:28px;height:28px;border-radius:50%;display:grid;place-items:center;flex:0 0 auto;border:1px solid #36515f;background:#0b1b24;color:#8fa5b0;font-size:12px;font-weight:900}
...
.pp-progress-track{position:absolute;display:none}
