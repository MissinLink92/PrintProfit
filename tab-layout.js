...
`;document.head.appendChild(style);
    ...
    function updateModelHub(){
      ...
      const q=id=>document.getElementById(id);const file=q('file'),status=q('status'),mat=q('material'),used=q('materialUsed');const h=q('ppPrintTimeHours'),m=q('ppPrintTimeMinutes'),legacy=q('printHours');const fileName=file?.files?.[0]?.name||'—';q('ppModelFile').textContent=fileName;
